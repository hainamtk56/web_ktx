...
<?php
	if (isset($_GET['timkiemcsvc_phong'])) {
    $search = $_GET['timkiemcsvc_phong'];
    $sql1="SELECT  ID,MaPhong, cosovatchat.MaCSVC, cosovatchat.TenCSVC, SoLuong, GhiChu FROM csvc_phong INNER JOIN cosovatchat ON csvc_phong.MaCSVC=cosovatchat.MaCSVC WHERE TenCSVC LIKE '%$search%' OR MaPhong LIKE '%$search%' ORDER BY MaPhong";
...
?>
	<tbody>
        ...
        <td><a href="index.php?action=suacsvc_phong&ID=<?php echo $row1['ID']?>"><button class='btn-sua'>Sửa</button></a></td>
        <td><a onclick="confirmDelete('<?php echo $row1['ID'] ?>')"><button class='btn-xoa'>Xóa</button></a></td>
  </tr>
	</tbody>
    <?php } } ?>
</table>
